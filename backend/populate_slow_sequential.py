"""
Slow Sequential Pipeline: Run ONE query completely, wait, then next
Tab 1: DECOMPOSE -> Tab 2: DISCOVER -> Tab 3: ANALYZE -> Tab 4: SETUP -> Tab 5: VALIDATE
Wait 60 seconds between queries to let LLM providers fully recover.
"""
...
async def main():
    """Run ONE query at a time, wait 60s between queries."""
    print("\n" + "="*100)
    print("  SLOW SEQUENTIAL PIPELINE: One query at a time, 60s wait between")
    print("="*100)
    print(f"\nRunning {len(QUERIES)} business ideas slowly...")
    ...
    for i, idea in enumerate(QUERIES, 1):
        ...
        # WAIT 60 seconds between queries for LLM providers to recover
        if i < len(QUERIES):
            print(f"\n[WAITING 60s before next query to let LLM providers recover...]")
